...
class GoLogin(object):
            ...
            for orbita_browser in os.listdir(browser_gologin):
                if not orbita_browser.endswith('.zip') and not orbita_browser.endswith(
                    ...
                    if not os.path.exists(self.executablePath) and not orbita_browser.endswith(
                        ...
                        self.executablePath = os.path.join(home, browser_gologin, orbita_browser,
    ...
    def getProfile(self, profile_id=None):
        ...
        pref_file = os.path.join(self.profile_path, 'profile.json')
        with open(pref_file, 'r', encoding="utf-8") as pfile:
    ...
    def create(self, options={}):
        ...
        response['id'] = profile.get('name')
        self.profile_path = os.path.join(self.tmpdir, 'gologin_' + response.get('id'))
        print(self.profile_path)
        if not os.path.exists(self.profile_path):
            os.makedirs(self.profile_path)
        with open(os.path.join(self.profile_path, 'profile.json'), "w") as json_file:
